  ...
  (if (pair? lst)
    (let ((obj (car lst)))
      (if (and (pair? obj) (c #'unquote-splicing (car obj)))
        (if (eq? nest 0)
          (if (cdr lst)
            (list '##arete#append1 (cadr obj) (qq-list c (cdr lst) nest))
            (cadr obj))
          (list #'cons* (list #'list (list #'quote #'unquote-splicing) (qq-object c obj (fx- nest 1)) (qq-list c (cdr lst) nest)))
          )
          ;(list #'cons* (list #'cons* (list #'quote #'unquote-splicing) (qq-list c (cdr lst) (fx- nest 1)))))
        (let ((chk (qq-list c (cdr lst) nest)) (qobj (qq-object c obj nest)))
          (if (and (pair? chk) (c #'cons* (car chk)))
            (append1 (list #'cons* qobj) (cdr chk))
            (if (pair? chk)
              (append1 (list #'cons* qobj) (if (null? chk) '(()) (list chk)))
              (begin
                (append1 (list #'cons* qobj) (if (null? chk) '(()) chk))))))
      ))
    (begin
      (if (null? lst) 
        '()
...
(define (qq-element c lst nest)
  (if (c #'unquote (car lst))
    (if (eq? nest 0)
      (cadr lst)
      (qq-list c lst (fx- nest 1)))
    (if (or (c #'quasiquote (car lst)) (eq? (car lst) 'quasiquote))
      (qq-list c lst (fx+ nest 1))
      (qq-list c lst nest))))
         
(define (qq-object c object nest)
  (if (pair? object)
