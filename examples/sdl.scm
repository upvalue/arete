  ...
  (when state-dirty
    ...
    #;(let ((grid (State/grid state)))
      (let loop ((x 0)
                 (y 0))
;        (print x y)
        (sdl:draw-line 5 5 (- *width* 5) 5)
        (sdl:draw-line 5 (- *height* 40) (- *width* 5) (- *height* 40))
        (sdl:draw-line 5 (- *height* 40) (- *width* 5) (- *height* 40))
        (sdl:draw-line (- *width* 5) 5 (- *width* 5) (- *height* 40))
        (unless (= y (grid-height grid))
          (if (= x (grid-width grid))
            (loop 0 (+ y 1))
            (begin
              (sdl:set-color 255 255 255)
              (sdl:draw-line (+ (* x *tile-width*) 5) 5 (+ (* x *tile-width*) 5) (- *height* 40))
              (sdl:draw-line 5 (* (- y 1) *tile-height*) (- *width* 5) (* (- y 1) *tile-height*))
              (loop (+ x 1) y)
              #;(let* ((xc (* x *tile-width*))
                     (yc (* y *tile-height*))
                     (w (+ xc *tile-width*))
                     (h (+ yc *tile-height*)))
                (sdl:set-color 255 255 255)
                (sdl:draw-rect xc yc w h)
                (print xc yc w h)
                (when (= (get-cell grid x y) 1)
                  (sdl:set-color 255 180 0)
                  (print "fill rect from " (+ xc 1) (+ yc 1) (- w 1) (- h 1))
                  (sdl:fill-rect (+ xc 1) (+ yc 1) (- *tile-width* 1) (- *tile-height* 1)))
              (sdl:set-color 0 0 0)
              (loop (+ x 1) y)))))))
