...
- Optional counters record applied rewrites for inline-let, named-loop,
  ...
  primitive folds, false unspecified `if`, and named-loop refusal categories.
...
- There are no binding records, reference counts, assignment facts, effect
  summaries, or structured refusal reasons yet.
...
- Conditional simplification is intentionally narrow: it does not reason about
  effects, bindings, or general predicates yet.
- The current context model is still just `in-lambda?`, `tail?`, and a small
  lexical primitive-shadow set.
...
- Introduce real contexts: value, effect, test, tail, and application.
