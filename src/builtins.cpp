...
Value fn_eval(State& state, size_t argc, Value* argv, void* v) {
  ...
  AR_FN_ARGC_BETWEEN(state, argc, 1, 2);
  SValue env = argc == 2 ? argv[1] : C_FALSE, exp = argv[0];
...
Value fn_raise(State& state, size_t argc, Value* argv, void* v) {
  ...
  AR_FN_EXPECT_TYPE(state, argv, 0, SYMBOL);
  AR_FN_EXPECT_TYPE(state, argv, 1, STRING);
  SValue tag = argv[0], message = argv[1], irritants = argc == 3 ? argv[2] : C_FALSE, exc;
...
#define _AR_FN_EXPECT_RECORD_ISA(state, expected_type, record) \
  if(((!((record).heap_type_equals(RECORD))))) { \
    std::ostringstream msg; \
    msg << fn_name << " expected a record of type " << \
      (expected_type).record_type_name().string_data() << " but got a " << (record).type(); \
    return (state).type_error(msg.str()); \
  } else if((!((record).record_isa((expected_type))))) { \
    std::ostringstream msg; \
    msg << fn_name << " expected a record of type " << \
...
Value fn_record_set(State& state, size_t argc, Value* argv, void* v) {
  ...
  if(argv[2].fixnum_value() >= argv[1].record_field_count()) {
    std::ostringstream os;
    os << "attempt to access field " << argv[2] << " in a record with only " <<
      argv[1].record_field_count() << " fields";
    return state.type_error(os.str());
  }
