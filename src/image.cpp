  ...
  void update_pointers(HeapValue* heap) {
    ...
    switch(heap->get_type()) {
      ...
      case VMFUNCTION: {
        ...
        if(!reading && static_cast<VMFunction*>(heap)->get_header_bit(Value::VMFUNCTION_NATIVE_VM_BIT)) {
          static_cast<VMFunction*>(heap)->unset_header_bit(Value::VMFUNCTION_NATIVE_VM_BIT);
        }
